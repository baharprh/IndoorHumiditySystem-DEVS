...
int main(int argc, char* argv[]) {
    std::string input_path = (argc > 1) ? std::string(argv[1]) : "input_data/humidity_plant_rh_profile.txt";
    double sim_time = (argc > 2) ? std::stod(argv[2]) : 8.0;
    std::string log_path = (argc > 3) ? std::string(argv[3]) : "logs/coupled_humidity_plant.csv";

    auto model = std::make_shared<humidity::testing::HumidityPlantExperiment>(
        "HumidityPlantExperiment", input_path.c_str()
    );
    ...
    std::cout << "HumidityPlant experiment finished." << std::endl;
